...
//    [self importKnownKeys];
    ...
    XCTAssertNotNil(privateKeyData, @"Private key data should not be nil.");
    NSData *publicKeyData = [QredoCrypto getKeyDataForIdentifier:publicKeyName];
    XCTAssertNotNil(publicKeyData, @"Public key data should not be nil.");
    ...
    NSLog(@"Public Key PKCS#1 Data: %@.", [QredoLogging hexRepresentationOfNSData:publicKeyData]);
    NSLog(@"Private Key PKCS#1 Data: %@.", [QredoLogging hexRepresentationOfNSData:privateKeyData]);
    NSLog(@"Modulus: %@.", [QredoLogging hexRepresentationOfNSData:qredoPrivateKey.modulus]);
    NSLog(@"Public exponent: %@.", [QredoLogging hexRepresentationOfNSData:qredoPrivateKey.publicExponent]);
    NSLog(@"Private exponent: %@.", [QredoLogging hexRepresentationOfNSData:qredoPrivateKey.privateExponent]);
}
...
- (void)generateRandomKeys
{
    ...
    NSAssert(success, @"RSA key generation failed.");
}
...
- (void)importKnownKeys {
    ...
    NSData *publicKeyData = [NSData dataWithBytes:publicKeyDataArray length:sizeof(publicKeyDataArray) / sizeof(uint8_t)];
    ...
    NSData *privateKeyData = [NSData dataWithBytes:privateKeyDataArray length:sizeof(privateKeyDataArray) / sizeof(uint8_t)];

    // Import the public key
    BOOL success = [QredoCrypto importKeyData:publicKeyData keyLengthBits:keySizeBits keyIdentifier:publicKeyName isPrivate:NO persistInAppleKeychain:persistKey];
    XCTAssertTrue(success, @"Public Key import failed.");

    // Import the private key
    success = [QredoCrypto importKeyData:privateKeyData keyLengthBits:keySizeBits keyIdentifier:privateKeyName isPrivate:YES persistInAppleKeychain:persistKey];
    XCTAssertTrue(success, @"Private Key import failed.");
}
...
- (void)testPSSSignAndVerifyMessageMultiple
{
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        @catch (NSException *exception) {
            NSLog(@"Failed on iteration %d", i);
            @throw exception;
        }
    }

    NSLog(@"Failed %d out of %d signatures + verification. %.2f%% failure rate", failed, iterations, 100.0 * failed/iterations);
