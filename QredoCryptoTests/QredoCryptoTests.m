...
//  LinguaFranca-iOS
//
//  Created by David Hearn on 01/09/2014.
...
- (void)testRsaKeyGenEncryptOaepAndDecrypt
{
    ...
    BOOL success = [QredoCrypto generateRsaKeyPairOfLength:keySizeBits publicKeyIdentifier:publicKeyIdentifier privateKeyIdentifier:privateKeyIdentifier persistInAppleKeychain:YES];
    XCTAssertTrue(success, "RSA key generation failed.");
    
    // Confirm keys are present
    NSData *keyData = [QredoCrypto getKeyDataForIdentifier:publicKeyIdentifier];
    XCTAssertNotNil(keyData, @"Public Key data should not be nil.");
...
- (void)testRsaKeyGenEncryptNoPaddingAndDecrypt_AllZeroes
{
    ...
    SecKeyRef publicKeyRef = [QredoCrypto getRsaSecKeyReferenceForIdentifier:publicKeyIdentifier];
    LogDebug(@"Plaintext data (%lu bytes = %lu bits) %@\n", plainTextData.length, (plainTextData.length * 8), [QredoLogging hexRepresentationOfNSData:plainTextData]);
    NSData *encryptedData = [QredoCrypto rsaEncryptPlainTextData:plainTextData padding:padding keyRef:publicKeyRef];
    XCTAssertNotNil(encryptedData, @"Encrypted data should not be nil.");
    
    SecKeyRef privateKeyRef = [QredoCrypto getRsaSecKeyReferenceForIdentifier:privateKeyIdentifier];
    LogDebug(@"Encrypted data (%lu bytes = %lu bits) %@\n", encryptedData.length, (encryptedData.length * 8), [QredoLogging hexRepresentationOfNSData:encryptedData]);
    NSData *decryptedData = [QredoCrypto rsaDecryptCipherTextData:encryptedData padding:padding keyRef:privateKeyRef];
...
- (void)testRsaKeyGenEncryptNoPaddingAndDecrypt_Multiple
{
    ...
    for (int i = 0; i < numberOfTests; i++)
    {
        ...
        NSLog(@"Modulus: %@.", [QredoLogging hexRepresentationOfNSData:publicKey.modulus]);
        NSLog(@"Public Exponent: %@.", [QredoLogging hexRepresentationOfNSData:publicKey.publicExponent]);
        NSLog(@"Data to encrypt: %@.", [QredoLogging hexRepresentationOfNSData:plainTextData]);
        ...
        if(encryptedData == nil)
        {
            ...
            NSLog(@"Skipping test iteration %d due to encrypted data being nil.", i);
            continue;
        }
        ...
        NSLog(@"Completed test iteration %d.", i);
    }
}
...
-(void)testRsaDecryptCipherTextData_BouncyCastleEncrypted
{
    ...
    BOOL importSuccess = [QredoCrypto importKeyData:keyData keyLengthBits:keyLengthBits keyIdentifier:privateKeyIdentifier isPrivate:isPrivateKey persistInAppleKeychain:persistKey];
    XCTAssertTrue(importSuccess, @"Private key import failed.");
...
- (void)testDeleteAllKeysInAppleKeychain
{
    ...
    NSInteger keySizeBits = 1024;
    BOOL success = [QredoCrypto generateRsaKeyPairOfLength:keySizeBits publicKeyIdentifier:publicKeyIdentifier1 privateKeyIdentifier:privateKeyIdentifier1 persistInAppleKeychain:YES];
    XCTAssertTrue(success, "RSA key generation failed.");
    ...
    success = [QredoCrypto generateRsaKeyPairOfLength:keySizeBits publicKeyIdentifier:publicKeyIdentifier2 privateKeyIdentifier:privateKeyIdentifier2 persistInAppleKeychain:YES];
    XCTAssertTrue(success, "RSA key generation failed.");
    ...
    success = [QredoCrypto deleteAllKeysInAppleKeychain];
...
- (void)testDeleteKeyInAppleKeychainWithIdentifier
{
    ...
    success = [QredoCrypto deleteKeyInAppleKeychainWithIdentifier:publicKeyIdentifier2];
    XCTAssertTrue(success == expectedSuccess, @"Delete keys should not have failed.");
...
- (void)testImportPublicKey
{
    ...
    BOOL success = [QredoCrypto importKeyData:keyData keyLengthBits:keySizeBits keyIdentifier:keyIdentifier isPrivate:isPrivate persistInAppleKeychain:persistKey];
    
    XCTAssertTrue(success, @"Key import failed.");
}
