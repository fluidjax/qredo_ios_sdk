...
@interface MQTTSession()<MQTTEncoderDelegate, MQTTDecoderDelegate>
{
    ...
    MQTTDecoder*         decoder;
    UInt16               txMsgId;
    __weak id<MQTTSessionDelegate> delegate;
...
streamSocketSecurityLevel:(CFStringRef)streamSocketSecurityLevel
...
- (void)connectToHost:(NSString*)ip port:(UInt32)port usingSSL:(BOOL)usingSSL {
    [self connectToHost:ip port:port usingSSL:usingSSL streamSocketSecurityLevel:nil trustValidator:nil];
}
...
usingSSLWithStreamSocketSecurityLevel:(CFStringRef)streamSocketSecurityLevel
       trustValidator:(MQTTSessionTrustValidator)trustValidator {
    [self connectToHost:ip port:port usingSSL:YES streamSocketSecurityLevel:streamSocketSecurityLevel trustValidator:trustValidator];
}
...
streamSocketSecurityLevel:(CFStringRef)streamSocketSecurityLevel
    ...
    if (usingSSL) {
        ...
        CFDictionaryRef sslSettings = nil;
        ...
        const void *vals[] = {
            streamSocketSecurityLevel ? streamSocketSecurityLevel : kCFStreamSocketSecurityLevelTLSv1,
            kCFNull,
            trustValidator ? kCFBooleanFalse : kCFBooleanTrue
        };
        
        sslSettings = CFDictionaryCreate(kCFAllocatorDefault, keys, vals, 3,
